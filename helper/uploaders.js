/*****************************************************************************
 * Converted to CommonJS — was written with ESM import/export, which crashes
 * under require() since package.json has no "type": "module".
 *****************************************************************************/
...
async function uploadToCatbox(filePath) {
  try {
    ...
    return { status: true, url: response.data.trim() };
  } catch (error) {
...
async function uploadToPomf2(filePath) {
  try {
    ...
    if (response.data.success && response.data.files && response.data.files.length > 0) {
      return { status: true, url: `https://pomf2.lain.la/f/${response.data.files[0].url}` };
    }
...
async function uploadToLitterbox(filePath, time = "1h") {
  try {
    ...
    return { status: true, url: response.data.trim(), expires: time };
  } catch (error) {
...
async function uploadToPixhost(filePath) {
  try {
    ...
    if (response.data.show_url) {
      ...
      if (match && match[1]) return { status: true, url: match[1] };
    }
...
async function uploadToTmpfiles(filePath) {
  try {
    ...
    if (response.data.status === "success") {
      ...
      return { status: true, url: directUrl, page_url: url };
    }
    throw new Error("Upload failed");
  } catch (error) {
