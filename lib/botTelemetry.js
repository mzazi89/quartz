...
async function authHeaders() {
  const key = await resolveBotApiKey();
  return { Authorization: `Bearer ${key}` };
}

async function reportStatus() {
  const BOT_API_KEY = await resolveBotApiKey();
  if (!BOT_API_KEY) {
    if (lastHeartbeatError !== "no-key") {
      lastHeartbeatError = "no-key";
      console.log("⚠ No bot API key available (shared DB lookup failed and BOT_API_KEY is not set) — cannot report status to the website.");
    }
    return;
  }
  try {
    ...
    if (!res.ok) {
      const msg = `Heartbeat rejected: HTTP ${res.status} (${STATUS_URL}) — check that the Bot API Key in the dashboard matches the bot's .env`;
...
async function pollControls() {
  ...
  try {
    const res = await fetch(CONTROL_URL, { headers: await authHeaders(), signal: AbortSignal.timeout(15000) });
    if (!res.ok) return;
