...
// ── Broadcast buttons ────────────────────────────────────────────────────────
// A broadcast used to go out as bare text in a grey bubble, which is the easiest
// thing in a busy group to scroll past. It now carries the bot's photo and two
// quick replies (see the broadcast case below).
//
// The button ids are real commands and they are built with the SESSION's own
// prefix, read from that session's settings.json: a session that has set
// `.setprefix !` would ignore a hard-coded `.menu`, and the button would look
// broken. Anything the admin supplies overrides these.
function sessionPrefix(conn) {
  const number = String((conn && conn.user && conn.user.id) || "")
    .split("@")[0]
    .split(":")[0]
    .replace(/[^0-9]/g, "");
  try {
    const raw = fs.readFileSync(`./database/sessions/${number}/settings.json`, "utf8");
    const settings = JSON.parse(raw);
    // "" is a real prefix (no-prefix mode), so test the type, not the truthiness.
    if (typeof settings.customPrefix === "string") return settings.customPrefix;
  } catch (e) {
    // No settings file yet — the default prefix is what the engine uses too.
  }
  return ".";
}

/** The buttons a broadcast carries when the admin did not choose any. */
function defaultBroadcastButtons(conn) {
  const p = sessionPrefix(conn);
  return [
    { id: `${p}menu`, text: "📜 Menu" },
    { id: `${p}plans`, text: "💳 Plans" },
  ];
}

/**
 * Whatever the admin asked for, in the shape sendButtonMessage expects.
 *
 * Returns the defaults when the admin sent nothing usable, so a broadcast never
 * goes out with no buttons at all. WhatsApp caps a button label at 20 characters;
 * a longer one is not rendered trimmed, it fails to send.
 */
function broadcastButtonsFrom(payload, conn) {
  const list = Array.isArray(payload && payload.buttons) ? payload.buttons : [];
  const cleaned = list
    .map((b) => ({
      id: String((b && (b.id || b.command)) || "").trim(),
      text: String((b && (b.text || b.label)) || "").trim().slice(0, 20),
    }))
    .filter((b) => b.id && b.text)
    .slice(0, 3);
  return cleaned.length ? cleaned : defaultBroadcastButtons(conn);
}
...
async function reportStatus() {
  try {
    ...
    // One row PER BOT — not one row called 'main'.
    //
    // bot_status is keyed by bot_id with ON CONFLICT (bot_id), so the single
    // insert that used to live here did not add a second row when a second bot
    // ran: it OVERWROTE the first one, and anything reading this table could
    // only ever see whichever heartbeat landed last.
    //
    // Session folders carry no profile of their own, so paired.json is what maps
    // a folder to its bot. A folder with no entry — a leftover from a pairing
    // that failed part way — belongs to the primary profile, the same rule used
    // everywhere else in this feature.
    const profiles = require("./profiles");
    const { loadJSON } = require("../helper/function");
    let paired = [];
    try {
      paired = loadJSON("./database/paired.json", []);
    } catch {}
    const ownerOf = (n) => {
      const e = (Array.isArray(paired) ? paired : []).find(
        (s) => String(s.number) === String(n)
      );
      return e ? profiles.profileOf(e) : profiles.primary().id;
    };
    ...
    for (const profile of profiles.list()) {
      const mine = sessionNumbers.filter((n) => ownerOf(n) === profile.id);
      const meta = {};
      for (const n of mine) {
        if (devicesMeta[n]) meta[n] = devicesMeta[n];
      }

      // This profile's OWN command count. `rs.count` used to be written here for
      // every profile in the loop, so each row carried whatever the single
      // registry-wide number happened to be — with two profiles configured, both
      // rows reported the same figure and neither was necessarily right.
      // countsByProfile is already computed and already keyed by profile id; an
      // empty-profile row counts under the primary because commandProfile()
      // resolves it that way.
      const commandCount =
        rs.countsByProfile && Object.prototype.hasOwnProperty.call(rs.countsByProfile, profile.id)
          ? rs.countsByProfile[profile.id]
          : 0;
      ...
      await prisma.$executeRawUnsafe(
        `INSERT INTO bot_status (bot_id, online, version, uptime_seconds, telegram_online, whatsapp_sessions, command_count, last_sync_at, last_sync_error, session_numbers, ip_address, devices_meta, last_seen_at)
         VALUES ($1, true, $2, $3, true, $4, $5, $6::timestamp, $7, $8, $9, $10, CURRENT_TIMESTAMP)
...
async function pollControls() {
  try {
    await ensureTables();
    // Only claim requests aimed at a bot this process actually serves.
    //
    // A request naming a bot that is not running here stays pending rather than
    // being picked up and paired by the wrong one. For WhatsApp that is not a
    // mistake you can quietly undo — the code would be issued against the wrong
    // session and the user would have no way to tell from the website.
    //
    // The ids are stripped to [A-Za-z0-9_-] before being interpolated, because
    // they come from a settings field and Prisma's raw binding cannot carry an
    // array into an IN list.
    const profiles = require("./profiles");
    const served = profiles
      .list()
      .map((p) => String(p.id).replace(/[^A-Za-z0-9_-]/g, ""))
      .filter(Boolean);
    const targets = served.length ? served.map((id) => `'${id}'`).join(",") : "''";
      ...
      WHERE id IN (
        SELECT id FROM bot_control
         WHERE status = 'pending'
           AND (bot_id IS NULL OR bot_id = '' OR bot_id IN (${targets}))
         ORDER BY id ASC LIMIT 10
      )
      RETURNING id, action, payload, bot_id
    `);

    for (const control of controls) {
      try {
        // Carry the claimed target into the payload.
        //
        // The website names its bot in the bot_id COLUMN, and the claim above
        // uses it — but executeControl only ever sees the payload, so without
        // this the pairing would look targeted and still be recorded under
        // whatever profile the Telegram user last picked. A payload that names
        // its own bot wins, being the more specific of the two.
        const payload = { ...(control.payload || {}) };
        if (!payload.bot && control.bot_id) payload.bot = control.bot_id;

        const result = await executeControl(control.action, payload);
...
async function executeControl(action, payload) {
  switch (action) {
    ...
    case "broadcast": {
      ...
      const WA = require("../whatsapp");
      const { sendButtonMessage } = require("./buttons");
      let sent = 0;
      let sessions = 0;
      for (const [, conn] of WA.activeSessions || []) {
        sessions++;
        try {
          // One call per session, not per group: the prefix — and therefore the
          // buttons' ids — belongs to the session.
          const buttons = broadcastButtonsFrom(payload, conn);
          const image = payload.image ? { url: String(payload.image) } : null;
          const groups = await conn.groupFetchAllParticipating();
          for (const gid of Object.keys(groups || {})) {
            // Carries the bot's photo (see lib/replyImage.js) plus the buttons.
            // sendButtonMessage falls back to plain text on its own if the
            // interactive send is refused, so a broadcast still lands either way.
            await sendButtonMessage(conn, gid, {
              text: payload.message,
              footer: config.signature || `© ${config.botName}`,
              buttons,
              image,
            });
            sent++;
    ...
    case "pair": {
      ...
      // Which bot is this pairing for? The website names one when it queues the
      // request. Failing here on an unknown name is deliberate: without it the
      // pairing would succeed into the primary profile and look fine while
      // landing on the wrong bot.
      const profiles = require("./profiles");
      if (payload && payload.bot) {
        if (!profiles.setPendingProfile(digits, payload.bot)) {
          throw new Error(
            'Unknown bot "' + payload.bot + '" — check the bot_profiles setting'
          );
        }
      }
