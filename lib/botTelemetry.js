...
async function executeControl(action, payload) {
  switch (action) {
    ...
    case "unpair": {
      ...
      // mode "delete" removes the DB row entirely (full wipe vs soft unlink)
    ...
    // Pause / resume a paired number from the website.
    // { number, accountId, paused: true|false }
    case "pause": {
      const { number, accountId, paused } = payload || {};
      const digits = String(number || "").replace(/\D/g, "");
      if (!digits || digits.length < 10 || digits.length > 15) {
        throw new Error("Invalid phone number");
      }
      ...
      const sessions = loadJSON("./database/paired.json", []);
      const entry = sessions.find((s) => s.number === digits);
      if (accountId && (!entry || String(entry.userId) !== String(accountId))) {
        throw new Error("This number is not linked to your account");
      }
      ...
      if (paused) {
        // pause: stop the live connection, mark the session paused
        if (entry) {
          entry.paused = true;
          entry.active = false;
        }
        ...
        try {
          const { prisma } = require("./botDb");
          await prisma.$executeRawUnsafe(
            `UPDATE "WhatsAppSession" SET status = 'PAUSED', "updatedAt" = CURRENT_TIMESTAMP WHERE "phoneNumber" = $1`,
            digits
          );
        } catch (e) {}
        return `Paused ${digits}`;
      }

      // resume: mark active and reconnect
      if (entry) {
        entry.paused = false;
        entry.active = true;
      }
      ...
      try {
        const { prisma } = require("./botDb");
        await prisma.$executeRawUnsafe(
          `UPDATE "WhatsAppSession" SET status = 'ACTIVE', "updatedAt" = CURRENT_TIMESTAMP WHERE "phoneNumber" = $1`,
          digits
        );
      } catch (e) {}
      if (!WA.activeSessions.has(digits)) {
        WA.connectToWhatsApp(digits, entry ? entry.userId : null).catch((e) =>
          console.error(`Resume failed ${digits}:`, e.message)
        );
      }
      return `Resumed ${digits}`;
    }
