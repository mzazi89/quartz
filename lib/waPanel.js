...
// ─── Buttons for the replies that ask the reader to DO something ─────────────
// A prompt that ends in "reply .cancel to abort" is asking for a typed command
// where one tap would do, and the same is true of a failed order that has to be
// started again. These build the three that recur; the ids are real commands in
// this session's prefix.
function cancelButton(prefix) {
  return {
    name: 'quick_reply',
    buttonParamsJson: JSON.stringify({ display_text: '❌ Cancel order', id: `${prefix}cancel` }),
  };
}

function retryButton(prefix) {
  return {
    name: 'quick_reply',
    buttonParamsJson: JSON.stringify({ display_text: '🖥 Try again', id: `${prefix}panel` }),
  };
}

function menuQuickButton(prefix) {
  return {
    name: 'quick_reply',
...
// `buttons` is optional and, when given, turns this reply into an interactive
// message — which also carries the bot's photo, since sendInteractiveMessage
// attaches it (lib/replyImage.js). A refused interactive send falls back to the
// plain text below rather than losing the reply.
async function sendText(mzazi, sender, txt, buttons = null) {
  if (Array.isArray(buttons) && buttons.length) {
    try {
      await sendInteractiveMessage(mzazi, sender, {
        title: '🖥 QUARTZ PANEL RESELLER',
        text: txt,
      ...
      // Fall through: the text still has to reach them.
    }
  }
...
async function createPanelForClient({ mzazi, sender, prefix, resellerPhone, username, phone, size }) {
  ...
  try {
    ...
    // A failed order is exactly where a button earns its place: the next thing
    // this reseller wants is to try again, not to retype the command.
    await sendText(
      mzazi,
      sender,
      `❌ *Panel creation failed:* ${e.message}\n\n` +
        `If the username is already taken on the panel, pick a different one.`,
      [retryButton(prefix), menuQuickButton(prefix)]
    );
    return false;
  }
}
...
async function handleCommand(ctx) {
  ...
  if (!reseller) {
    ...
    // A password has to be typed, so the only honest button here is the way out
    // of the flow: the menu.
    await sendText(
      mzazi,
      sender,
      `🔐 *RESELLER ACCESS*\n\n` +
