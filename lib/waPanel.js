...
async function handlePlainInput({ mzazi, sender, budy, senderPhone, prefix, isKnownCommand = false, isPanelCommand = false }) {
  ...
  if (!client) {
    // .panel / .unlimited / .cancel own this state — leave them to it.
    ...
    // corrected rather than the whole size selection repeated.
    await sendText(
      mzazi,
      sender,
