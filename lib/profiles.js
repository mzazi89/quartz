...
// ── Pending pairings ─────────────────────────────────────────────────────────
// A pairing is requested now and completed when the user types the code, which
// can be minutes later. The Telegram path knows the profile up front; the
// WEBSITE path goes through no Telegram user at all, so without this it would
// fall back to the primary profile and the number would land on the wrong bot.
//
// Persisted rather than kept in memory: a restart between "code issued" and
// "code entered" is entirely possible on Pterodactyl, and losing this would
// silently mis-route a pairing with no error anywhere.
const PENDING_FILE = './database/pending-profiles.json';

function loadPending() {
  try {
    const raw = loadJSON(PENDING_FILE, {});
...
/** Remember which bot a number is being paired to. */
function setPendingProfile(phone, profileId) {
  const number = String(phone == null ? '' : phone).replace(/\D/g, '');
  if (!number || !byId(profileId)) return false;
  const all = loadPending();
  all[number] = { profile: String(profileId), at: Date.now() };
  try {
    saveJSON(PENDING_FILE, all);
  } catch (err) {
    console.warn('[profiles] could not persist the pending pairing:', err.message);
  }
  return true;
}

/**
 * Read AND clear — deliberately one-shot.
 *
 * A leftover entry must never influence a later, unrelated pairing of the same
 * number, which is exactly what a read-without-clear would do.
 */
function takePendingProfile(phone) {
  const number = String(phone == null ? '' : phone).replace(/\D/g, '');
  ...
  try {
    ...
    // The in-memory copy is already cleared, so this pairing is still correct.
  }
  return byId(entry.profile) ? entry.profile : null;
}
