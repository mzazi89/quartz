...
// ─── slanted text ────────────────────────────────────────────────────────────
// Mathematical Italic. MZAZI XMD presents its replies in the italic letterforms;
// QUARTZ XD does not, and the two bots share this file, so it is switched on by
// settings rather than by which repo it is in.
//
// The mapping is NOT a straight offset. Exactly one slot in the italic range was
// never assigned — U+1D455, which is where the lowercase "h" would land — and the
// italic h lives at U+210E instead. A mechanical offset would print a tofu box for
// every h in every reply, which is the kind of bug that only shows up in
// production. Letters after h are unaffected: the gap is at h, so i onwards sit at
// their natural positions.
//
// Digits have no italic form in Unicode and are left alone, as is anything that is
// not a Latin letter: box drawing, emoji, punctuation, other scripts.
const ITALIC_UPPER = 0x1d434; // 𝐴
const ITALIC_LOWER = 0x1d44e; // 𝑎
const ITALIC_H = 0x210e; // ℎ — the one that is not in the range

function italicChar(ch) {
  const code = ch.charCodeAt(0);
  if (code >= 65 && code <= 90) return String.fromCodePoint(ITALIC_UPPER + (code - 65));
  if (code >= 97 && code <= 122) {
    if (code === 104) return String.fromCodePoint(ITALIC_H);
    return String.fromCodePoint(ITALIC_LOWER + (code - 97));
  }
  return ch;
}

// Things a user might copy in order to USE them. A slanted URL is no longer that
// URL — it will not open and it will not resolve — so these stretches are stepped
// over rather than converted, which is the one exception to "everything it sends".
// Kept short and anchored to real shapes, so that ordinary text like "file.js" or
// "menu.jpg" is not mistaken for an address.
const KEEP_USABLE =
  /(?:https?:\/\/|www\.)\S+|\b[\w.+-]+@[\w-]+\.[\w.-]+\b|\b[\w-]+(?:\.[\w-]+)*\.(?:com|net|org|shop|io|dev|app|co|ke|me|info|xyz|site|link|online|store)\b(?:\/\S*)?/gi;

/** Slant the Latin letters in a run of text, leaving everything else alone. */
function slant(text) {
  return String(text).replace(/[A-Za-z]/g, italicChar);
}

/**
 * Slant a whole reply, except for the parts that have to stay copyable.
 *
 * Idempotent by construction: the output contains no ASCII letters, so running it
 * over its own result changes nothing. That matters because a reply can pass
 * through restyle() more than once — once where the command builds it and again on
 * the raw send.
 */
function italicize(text) {
  if (typeof text !== "string" || !text) return text;

  const re = new RegExp(KEEP_USABLE.source, "gi");
  let out = "";
  let at = 0;
  let m;
  while ((m = re.exec(text)) !== null) {
    out += slant(text.slice(at, m.index)) + m[0];
    at = m.index + m[0].length;
    if (m.index === re.lastIndex) re.lastIndex += 1;
  }
  return out + slant(text.slice(at));
}
...
const RESPONSE_STYLES = {
  ...
  xmd: [
    ...
    ["⚠️", "❗"],
    ["•", "▸"],
    // NOTHING THAT TOUCHES THE MENU FRAME MAY BE ADDED HERE.
    //
    // There used to be a ["━", "┄"] entry, rewriting every U+2501 to U+2504.
    // MZAZI XMD's menus (lib/menus.js) are built from U+2501 — the header rule,
    // every section rule and the footer — so that one entry turned the entire
    // frame into dashed lines. The frame's characters are
    //   ━ (U+2501)  ┃  ┣  ╭  ╮  ╰  ╯  〔  〕  🔹  🔗
    // and none of them may appear as a `from` key in this table.
    //
    // QUARTZ XD selects `classic` (an empty table), so nothing here applies to
    // it — this is kept identical to the other bot's copy of this file so the
    // trap cannot survive in one of them and not the other.
  ],
};
...
function build() {
  ...
  // Opt-in, and off unless a bot asks for it: the file is shared, so the default
  // has to be the quiet one.
  const slanted = t.slantText === true;
  const signature = `© ${name} ${badge}`;
  const signatureUpper = `© ${name.toUpperCase()} ${badge}`;
  ...
  return {
    ...
    // Slanted here rather than at send time: the footer is passed to the sender
    // separately from the text, so it never goes through restyle() and would
    // otherwise be the one part of a reply still in plain letters.
    signature: slanted ? italicize(signature) : signature,
    signatureUpper: slanted ? italicize(signatureUpper) : signatureUpper,
...
function restyle(text) {
  ...
  // LAST, deliberately. Every rule above matches plain ASCII — this bot's name,
  // the other bot's name, the word PLANS — and not one of them would match text
  // that had already been turned into italic letterforms.
  if (identity.slantText) out = italicize(out);
...
module.exports = {
  ...
  ornament,
  restyle,
  // Exported so the character mapping and the protected-span handling can be
  // exercised directly, without having to go through a whole reply.
  italicize,
  slant,
  italicChar,
  bindSocket,
