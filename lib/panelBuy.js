...
async function initializePanelPayment(pkg, telegramId, user, amountOverride = null) {
  ...
  const email = `${telegramId}@mzazitechxmd.bot`;
...
function normalizeWaPhone(raw) {
  ...
  // Kenya is 254 followed by nine digits. This used to require the 2547 range
  // specifically, which quietly locked out every number on the newer 2541 (011x)
  // ranges — such a reseller could never activate, and the bot told them their
  // password was wrong.
...
// ─── Reseller activation ──────────────────────────────────────────────────────
// A password is matched case-INSENSITIVELY against the stored value. The admin
// chooses these strings now, so uppercasing the typed input would fail against any
// password that is not already upper case — the same "it ignores my password"
...
async function activateReseller(code, phone) {
  ...
  if (!normPhone) return { ok: false, error: 'PHONE', seen: String(phone == null ? '' : phone) };

  const codeStr = String(code || '').trim();
  if (!codeStr) return { ok: false, error: 'INVALID' };

  const rows = await prisma.$queryRawUnsafe(
    `SELECT id, status, activated_by FROM reseller_passwords WHERE UPPER(code) = UPPER($1)`, codeStr
  );
  ...
  if (row.status === 'active') {
    // Re-entering your own password is not an attack, it is a person checking. Say
    // so instead of "used on another number", which would be untrue and alarming.
    if (String(row.activated_by || '') === normPhone) return { ok: true, already: true };
...
/** The reseller row bound to a number, or null. */
async function getReseller(phone) {
  await botDb.ensureTables();
  const normPhone = normalizeWaPhone(phone);
  if (!normPhone) return null;
  const rows = await prisma.$queryRawUnsafe(
    `SELECT id, code, status, activated_by FROM reseller_passwords
     WHERE status = 'active' AND activated_by = $1 LIMIT 1`,
    normPhone
  );
  return (rows || [])[0] || null;
}
...
async function isReseller(phone) {
  return (await getReseller(phone)) !== null;
}
