...
// Scoped to the bot profile this user is currently working in, so QUARTZ XD and
// MZAZI XMD never show each other's numbers. This is the single choke point for
// the device list, which is why the whole feature only needed one change here.
//
// With one profile configured — the default — filterForUser is a no-op and this
// returns exactly what it always did.
function getUserSessions(telegramId) {
  const sessions = loadJSON('./database/paired.json', []);
  const owned = sessions.filter(s => String(s.userId) === String(telegramId));
  return profiles.filterForUser(owned, telegramId);
}

// ─── Get a user's sessions across EVERY profile ───────────────────────────────
// Used where the count must stay whole: subscription limits must not change
// meaning just because a second bot was switched on. Billing is a business
// decision, not a side effect of this feature.
function getUserSessionsAllProfiles(telegramId) {
  const sessions = loadJSON('./database/paired.json', []);
