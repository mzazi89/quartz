...
async function ensureTables() {
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS bot_commands (
      ...
      code TEXT DEFAULT '',
      -- Which bot this command belongs to. '' means it predates bot profiles and
      -- therefore belongs to the PRIMARY profile — the same default-safe rule
      -- used in lib/profiles.js, so no existing command disappears when this
      -- column appears.
      profile VARCHAR(64) DEFAULT '',
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )
  `);
  // An existing database already has this table, so the CREATE above is a no-op
  // for it and the column has to be added on its own.
  await prisma.$executeRawUnsafe(`
    ALTER TABLE bot_commands ADD COLUMN IF NOT EXISTS profile VARCHAR(64) DEFAULT ''
  `);
  await prisma.$executeRawUnsafe(`
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS bot_control (
      ...
      -- Which bot this request is for. '' means "any bot may take it", which is
      -- the behaviour before bots were selectable — so every row that already
      -- exists stays claimable and nothing is stranded by this column appearing.
      bot_id VARCHAR(64) DEFAULT '',
  ...
  `);
  // bot_control predates selectable bots, so an existing database has the table
  // and needs the column added rather than created.
  await prisma.$executeRawUnsafe(`
    ALTER TABLE bot_control ADD COLUMN IF NOT EXISTS bot_id VARCHAR(64) DEFAULT ''
  `);
...
function mapRow(r) {
  return {
    ...
    code: r.code || '',
    // Empty string, not undefined, so callers can compare it directly.
    profile: r.profile || '',
  };
}
