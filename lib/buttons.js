...
//
// Every button message also carries a photo (see lib/replyImage.js), because this
// is the path nearly every command reply takes: mzazireply hands its text here.
// Attaching the picture at this one point is what makes "every command that has a
// button also has a picture" true without touching a single command body.
// ─────────────────────────────────────────────────────────────────────────────
const crypto = require('crypto');
const { proto, generateWAMessageFromContent, prepareWAMessageMedia } = require('@whiskeysockets/baileys');
const { replyImageFor, numberFromJid } = require('./replyImage');
...
async function sendButtonMessage(sock, jid, opts = {}) {
  const { text = '', footer = '© Bot', buttons = [] } = opts;

  // An image the caller chose always wins; a caller with none gets the bot's own.
  // Guarded, because a missing picture must never be the reason a reply fails:
  // on any failure this stays null and the message goes out as it always did.
  let image = opts.image || null;
  if (!image && buttons.length) {
    try {
      image = await replyImageFor({ botPhoneNum: numberFromJid(sock && sock.user && sock.user.id) });
    } catch (e) {
      image = null;
    }
  }
  ...
  const content = {
    interactiveMessage: {
      ...
      // Forwarded as the QUARTZ XD channel — the same identity case.js uses in
      // mzazireply. This is the path nearly every reply actually takes, because
      // mzazireply sends through here whenever it has buttons. Keep the two in
      // step: channel-branded plain replies next to generic-forwarded buttons
      // looks broken. serverMessageId is omitted on purpose (it names one post
      // in one channel).
      contextInfo: {
        forwardingScore: 2,
        isForwarded: true,
