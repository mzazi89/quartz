...
// message is `fromMe`, so `fromMe` cannot separate the owner's from ours. Recording
// the message ID alone was NOT enough — the echo did not always carry the id
// sendMessage returned, and the spam carried on — so the TEXT is recorded too. See
// lib/ownMessages.js.
...
You:`.trim();
    ...
    if (!isGroup && waPanel.hasPending(sender) && budy && budy.trim()) {
      ...
      const consumed = await waPanel.handlePlainInput({
        ...
        isOwnMessage: ownMessages.isOwnEcho(m.key?.id, budy),
      });
      if (consumed) return;
    }
